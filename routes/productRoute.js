...
//Create Product
router.post("/product", isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
...
//Update Product
router.put("/product/:id", isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
    ...
    if(product){
        ...
        //Delete Product
router.delete("/product/:id", isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
    const productId = req.params.id;
    const deletedProduct = await Product.findById(productId);
    if(deletedProduct){
       await deletedProduct.remove()
        return res.status(201).json({message : 'Product Deleted',  data: deletedProduct})
    }
